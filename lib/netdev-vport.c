...
set_tunnel_config(struct netdev *dev_, const struct smap *args, char **errp)
{
    ...
    if (tnl_cfg.csum == NETDEV_TNL_CSUM_DEFAULT) {
        if (!has_csum || strstr(type, "gre")) {
            tnl_cfg.csum = NETDEV_TNL_DEFAULT_NO_CSUM;
        } else {
            const char *dpif_type = netdev_get_dpif_type(dev_);

            if (dpif_type && strcmp("system", dpif_type) == 0) {
                /* Retain existing behavior with kernel datapath of no csum for
                 * IPv6 tunnels. */
                tnl_cfg.csum = NETDEV_TNL_DEFAULT_NO_CSUM;
            }
        }
